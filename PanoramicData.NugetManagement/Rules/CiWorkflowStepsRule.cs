...
public class CiWorkflowStepsRule : RuleBase
{
	...
	public override Task<RuleResult> EvaluateAsync(RepositoryContext context, CancellationToken cancellationToken)
	{
		...
		if (!hasRestore) missingSteps.Add("dotnet restore");
		if (!hasBuild) missingSteps.Add("dotnet build");
		if (!hasRelease) missingSteps.Add("--configuration Release");
		if (!hasPack) missingSteps.Add("dotnet pack");
