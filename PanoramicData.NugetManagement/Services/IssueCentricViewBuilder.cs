...
public static class IssueCentricViewBuilder
{
	...
	public static IssueCentricView Build(
		...
		=> Build(
			assessments.Select(a => new AssessedPackage(a.RepositoryFullName, a.Assessment)),
			canRemediate);

	/// <summary>
	/// Builds the issue-centric view from assessments that also identify the package each one is for,
	/// so a repository hosting several packages can be told apart where it appears more than once
	/// under the same rule.
	/// </summary>
	/// <param name="assessments">Per-package assessments.</param>
	/// <param name="canRemediate">
	/// Optional predicate indicating whether a given rule result can be auto-remediated. When null,
	/// every occurrence is treated as non-remediable (manual/AI only). The Web layer supplies the
	/// remediation registry here.
	/// </param>
	public static IssueCentricView Build(
		IEnumerable<AssessedPackage> assessments,
		Func<RuleResult, bool>? canRemediate = null)
	{
		var failures = assessments
			.SelectMany(a => a.Assessment.RuleResults
				.Where(r => !r.Passed)
				.Select(r => (a.RepositoryFullName, a.PackageId, Result: r)));
		...
		var issueClasses = failures
			...
			.Select(ruleGroup =>
			{
				...
				var instances = ruleGroup
					...
					.ThenBy(i => i.PackageId, StringComparer.OrdinalIgnoreCase)
					.ToList();
